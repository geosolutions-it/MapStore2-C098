...
import LayersUtils from '@mapstore/utils/LayersUtils';
import LocaleUtils from '@mapstore/utils/LocaleUtils';
import FileUtils from '@mapstore/utils/FileUtils';
import SelectShape from '@mapstore/components/import/SelectShape';
import styleUtilsImporter from '@mapstore/utils/StyleUtils';

class ShapeFileUploadAndStyle extends React.Component {
    static propTypes = {
        assetFeature: PropTypes.object,
    ...
    componentWillMount() {
        StyleUtils = styleUtilsImporter(this.props.mapType);
    }
    ...
    render() {
        return (
            ...
            <Row style={{textAlign: "center"}}>
                {
            this.props.selected
                ? <DropdownList data={this.props.layers} value={this.props.selected} onSelect={(value)=> this.props.onSelectLayer(value)} valueField={"id"} textField={"name"} />
            : this.props.assetFeature ? null : <SelectShape {...this.props.uploadOptions} errorMessage="shapefile.error.select" text={this.props.uploadMessage} onShapeChoosen={this.addShape} onShapeError={this.props.onShapeError}/>
            }
            </Row>
